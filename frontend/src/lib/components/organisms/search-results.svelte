<script lang="ts">
	...
	const { results, onClear, onOpenFile, onShowInFinder, class: className }: Props = $props();
...
<div class="bg-card mt-4 overflow-hidden rounded-xl border shadow-lg {className}">
	...
	<div class="max-h-96 overflow-y-auto">
		{#each results as result}
			<SearchResultItem {result} onOpen={handleOpenFile} onShowInFinder={handleShowInFinder} />
		{/each}
	</div>
</div>
