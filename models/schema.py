...
class IndexFolderRequest(BaseModel):
    ...
    recursive: bool = Field(True, description="Whether to index subdirectories recursively")
...
class GenerateNameRequest(BaseModel):
    ...
    context: Optional[str] = Field(None, description="Additional context for filename generation")
    preserve_extension: bool = Field(True, description="Whether to preserve the original file extension")
...
class IndexFolderResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    indexed_files: List[FileInfo] = Field(..., description="List of successfully indexed files")
    failed_files: List[str] = Field(..., description="List of files that failed to index")
...
class GenerateNameResponse(BaseModel):
    ...
    confidence_score: float = Field(..., description="Confidence score of the suggestion (0.0 to 1.0)")
    reasoning: Optional[str] = Field(None, description="Explanation of the naming decision")
